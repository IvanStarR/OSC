...
struct KV::Impl {
  ...
  std::unordered_map<std::string, std::optional<std::string>> mem; // tombstone = nullopt
  std::mutex mu;
  ...
  Impl(const KVOptions& o): opts(o) {
    ...
    if (rd.good()) {
      ...
      while (auto it = rd.next()) {
        if (it->flags == 1) mem[it->key] = it->value; // PUT
        else if (it->flags == 2) mem[it->key] = std::nullopt; // DEL
...
bool KV::init_storage_layout(){
  bool ok = ensure_dir(p_->opts.path) && ensure_dir(join_path(p_->opts.path, "wal")) && ensure_dir(join_path(p_->opts.path, "sst"));
...
std::optional<std::string> KV::get(std::string_view key){
  ...
  if (it == p_->mem.end()) return std::nullopt;
...
std::vector<RangeItem> KV::scan(std::string_view start, std::string_view end){
  ...
  std::vector<std::string> keys; keys.reserve(p_->mem.size());
  for (auto& [k,v]: p_->mem) if (v.has_value()) keys.push_back(k);

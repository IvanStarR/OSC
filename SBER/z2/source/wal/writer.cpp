...
list_wal_segments_sorted(const std::string &dir) {
  ...
  while (auto *ent = ::readdir(d)) {
    ...
    if (n.size() == 10 && n.substr(6) == ".wal") {
      bool digits = std::all_of(n.begin(), n.begin() + 6, [](unsigned char c) {
...
WalWriter::WalWriter(const std::string &wal_dir, bool use_uring,
                     unsigned uring_qd, uint64_t max_segment_bytes,
                     FlushMode flush_mode)
    : wal_dir_(wal_dir), use_uring_(use_uring),
      max_segment_bytes_(max_segment_bytes), flush_mode_(flush_mode) {
  if (wal_dir_.empty())
    return;
  ...
  if (use_uring_) {
    ...
    if (!uring_.initialized()) {
      use_uring_ = false;
      spdlog::warn(
          "liburing not available or init failed; falling back to POSIX I/O");
    } else {
...
bool WalWriter::open_or_rotate_if_needed(uint64_t next_bytes,
                                         uint64_t next_seqno) {
  if (seg_size_ + next_bytes + sizeof(WalRecordTrailer) > max_segment_bytes_) {
    if (!this->fsync_backend())
      return false;
...
bool WalWriter::append_put(uint64_t seqno, std::string_view k,
  ...
  WalRecordMeta m{static_cast<uint32_t>(k.size()),
                  static_cast<uint32_t>(v.size()), WAL_FLAG_PUT, seqno,
                  dummy_checksum(k, v)};
...
bool WalWriter::append_del(uint64_t seqno, std::string_view k) {
  std::string_view v{};
  WalRecordMeta m{static_cast<uint32_t>(k.size()), 0u, WAL_FLAG_DEL, seqno,
                  dummy_checksum(k, v)};
  return append_(m, k, v);
}
...
bool WalWriter::append_(const WalRecordMeta &m, std::string_view k,
  ...
  if (!this->open_or_rotate_if_needed(body + trailer_sz, m.seqno))
    return false;
  ...
  if (pad) {
    ...
    while (pad > 0) {
      const size_t chunk =
          static_cast<size_t>(std::min<uint64_t>(pad, sizeof(zeros)));
      ssize_t w = ::write(fd_, zeros, chunk);
      if (w < 0) {
        spdlog::error("WAL pad write failed");
        return false;
      }
  ...
  this->bytes_since_sync_ += used;

  // условный group-commit: ~1MiB полезных данных
...
bool WalWriter::write_vectored(const struct ::iovec *iov, int iovcnt) {
  if (use_uring_ && uring_.initialized()) {
    if (uring_.writev(fd_, iov, iovcnt))
      return true;
    // если не получилось — падаем на POSIX
  }
...
#else
  // на некоторых платформах fdatasync может отсутствовать — fallback на fsync
  return ::fsync(fd) == 0;
#endif
}

bool WalWriter::fsync_backend() {
  // io_uring fsync уже использует IORING_FSYNC_DATASYNC внутри
  // UringBackend::fsync()
  if (use_uring_ && uring_.initialized()) {
    if (flush_mode_ == FlushMode::FSYNC) {
      // у нас UringBackend всегда делает DATASYNC; для строго FSYNC
      // fallback-немся на POSIX
      return ::fsync(fd_) == 0;
    }
...
#ifdef __linux__
    // Записываем грязные страницы на диск, но не ждём журнала метаданных
    // После ряда SFR-вызовов всё же хорошо сделать fdatasync (мы делаем его по
    // group-commit условию)
    off_t end = ::lseek(fd_, 0, SEEK_END);
    if (end < 0)
      return false;
#ifdef SYNC_FILE_RANGE_WRITE
    if (::sync_file_range(fd_, 0, end, SYNC_FILE_RANGE_WRITE) != 0)
      return false;
#else
    // если вдруг нет макроса — используем fdatasync
    return posix_fdatasync(fd_);
#endif
    return true;
#else

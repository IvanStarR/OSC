...
// --- утилита: получить список сегментов в каталоге wal ---
...
WalWriter::WalWriter(const std::string& wal_dir, bool use_uring, unsigned uring_qd, uint64_t max_segment_bytes)
  : wal_dir_(wal_dir),
    ...
    max_segment_bytes_(max_segment_bytes)
{
  ...
  // определить текущий индекс (если есть файлы)
  auto files = list_wal_segments_sorted(wal_dir_);
  if (!files.empty()) {
    seg_index_ = std::stoull(files.back().substr(0,6));
  } else {
    seg_index_ = 0;
  }

  // открыть новый сегмент
  open_new_segment(seg_index_ + 1, /*start_seqno*/1);
}
...
bool WalWriter::open_or_rotate_if_needed(uint64_t next_bytes, uint64_t next_seqno) {
  if (seg_size_ + next_bytes > max_segment_bytes_) {
...
bool WalWriter::append_put(uint64_t seqno, std::string_view k, std::string_view v) {
  WalRecordMeta m{static_cast<uint32_t>(k.size()), static_cast<uint32_t>(v.size()),
...
bool WalWriter::append_del(uint64_t seqno, std::string_view k) {
  std::string_view v{};
  WalRecordMeta m{static_cast<uint32_t>(k.size()), 0u,
                  WAL_FLAG_DEL, seqno, dummy_checksum(k,v)};
  return append_(m,k,v);
}
...
bool WalWriter::append_(const WalRecordMeta& m, std::string_view k, std::string_view v) {
  ...
  uint64_t need = sizeof(m) + k.size() + v.size();
  if (!open_or_rotate_if_needed(need, m.seqno)) return false;
  ...
  if (!write_vectored(iov, 3)) return false;

  seg_size_ += need;
  bytes_since_sync_ += need;

...
static vector<string> find_repodata_dirs(const string& repoid) {
  ...
  for (auto& it : std::filesystem::directory_iterator(base)) {
    ...
    if (name.rfind(repoid + "-", 0) == 0) {
      ...
      if (std::filesystem::exists(repodata)) out.push_back(repodata);
    }
  }
  return out;
}
...
static bool repo_add_from_repodata_dir(Repo* repo, const std::string& dir) {
  ...
  // primary (обязателен)
  {
    ...
    if (!xf.fp) { spdlog::warn("cannot open {}", primary); return false; }
    if (repo_add_rpmmd(repo, xf.fp, nullptr, flags) != 0) { xfclose_any(xf); return false; }
    xfclose_any(xf);
  }
  // filelists (опционально)
  if (!filelists.empty()) {
    XFile xf = xfopen_any(filelists);
    if (xf.fp) { repo_add_rpmmd(repo, xf.fp, nullptr, flags); xfclose_any(xf); }
  }
  // other (опционально)
  if (!other.empty()) {

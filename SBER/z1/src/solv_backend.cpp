...
bool build_graphs_with_libsolv(const string& repoids_csv, const string& archs_csv, Graph& gr_runtime, Graph& gr_build, SolvStats& stats, int threads) {
    // 1) подготовка списка репо и архов
    ...
    // 2) создать pool и загрузить все repodata
    PoolGuard pg;
    Pool* pool = pg.pool;
    ...
    // 3) собрать список интересных solvables (id) для runtime и build
    ...
    // 4) параллельно обработаем requires → providers
    std::mutex pool_mx;// libsolv-пул используем под мьютексом
    std::unordered_set<std::pair<string, string>, EdgeHasher> seen_rt;
    std::unordered_set<std::pair<string, string>, EdgeHasher> seen_bd;

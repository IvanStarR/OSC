...
namespace gitproc { namespace io {

void ensure_dir(const fs::path& p){
  if (!fs::exists(p)) fs::create_directories(p);
}

void rotate_logs(const fs::path& base_path, std::uintmax_t max_bytes, int backups){
  std::error_code ec;
  if (!fs::exists(base_path, ec)) return;
  auto sz = fs::file_size(base_path, ec);
  if (ec || sz < max_bytes) return;

  // сдвиг .(backups-1) -> .backups, ... , .1 -> .2
  for (int i = backups-1; i >= 1; --i){
    fs::path src = base_path; src += "." + std::to_string(i);
    fs::path dst = base_path; dst += "." + std::to_string(i+1);
    if (fs::exists(src)) { std::error_code e2; fs::rename(src, dst, e2); }
  }
  // текущий -> .1
  fs::path first = base_path; first += ".1";
  { std::error_code e3; fs::rename(base_path, first, e3); }
  // создаём пустой файл

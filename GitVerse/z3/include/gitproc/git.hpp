...
// Простой git-враппер: путь или URL (http/ssh/git/file).
class GitRepo {
public:
  ...
  // pull с простым экспоненциальным backoff'ом при неудачах; возвращает true, если HEAD изменился
  ...
  // Контент-хэш: детект изменения по xxh3; кэшируем хэши
  bool has_unit_changed(const std::filesystem::path& unit_path, bool* head_changed = nullptr);
...
private:
  ...
  // для has_unit_changed: key = abs path, value = hex хэш

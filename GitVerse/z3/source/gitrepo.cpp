...
static int sysrun(const std::string &cmd) { return std::system(cmd.c_str()); }

GitRepo::GitRepo(std::string workdir) : workdir_(std::move(workdir)) {}

bool GitRepo::open_or_clone(const std::string &url_or_path,
  ...
  if (fs::exists(url_or_path) && fs::exists(fs::path(url_or_path) / ".git")) {
    std::string cmd =
        "git clone -q -b " + branch_ + " " + url_or_path + " " + workdir_;
    return sysrun(cmd) == 0;
  } else {
    std::string cmd =
        "git clone -q -b " + branch_ + " " + url_or_path + " " + workdir_;
...
bool GitRepo::pull_reset() {
  ...
  std::string reset =
      "git -C " + workdir_ + " reset -q --hard origin/" + branch_;
...
GitRepo::scan_services(const std::string &rel_dir) const {
...
GitRepo::read_file(const std::string &rel_path) const {
...
bool GitRepo::checkout_file_at(const std::string &commit,
                               const std::string &rel_file) {
  fs::path abs = fs::path(workdir_) / rel_file;
  fs::create_directories(abs.parent_path());
  std::string cmd = "git -C " + workdir_ + " show " + commit + ":" + rel_file +
                    " > " + abs.string();
  return sysrun(cmd) == 0;
}
...
ServicesDiff diff_sets(const std::vector<ServiceEntry> &old_list,
  ...
  for (auto &n : newn) {
    if (oldn.count(n)) {
      auto o = std::find_if(old_list.begin(), old_list.end(),
                            [&](auto &s) { return s.name == n; });
      auto nn = std::find_if(new_list.begin(), new_list.end(),
                             [&](auto &s) { return s.name == n; });

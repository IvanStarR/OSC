...
bool Watcher::poll_once() {
  ...
  if (head_changed || content_changed) {
    spdlog::info("[watch] change detected: reason={}", head_changed ? "head_changed" : "content_changed");
    if (on_change_) on_change_(resolved_path_);
    return true;
  }
  return false;
}

void Watcher::run_loop(std::chrono::milliseconds period, std::atomic_bool& stop_flag) {
  (void)poll_once();

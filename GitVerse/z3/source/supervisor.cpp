...
bool Supervisor::start(const Unit &u) {
  stop();
  if (ProcessRunner::start(u, run_dir_, logs_dir_) != 0)
    return false;
...
bool Supervisor::open_repo(const std::string &url_or_path,
...
bool Supervisor::sync_and_apply() {
  ...
  for (auto &name : scan_service_names())
    have.insert(name);

  std::set<std::string> was;
  for (auto &kv : last_unit_path_)
    was.insert(kv.first);
...
bool Supervisor::rollback_unit(const std::string &name,
  ...
  std::string cmd = "git -C \"" + repo_.root().string() + "\" show " + commit +
...
fs::path Supervisor::resolve_unit_path_by_name(const std::string &name) const {
  ...
  if (fs::exists(cand1))
    return fs::absolute(cand1);
  if (fs::exists(cand2))
    return fs::absolute(cand2);
...
std::vector<std::string> Supervisor::scan_service_names() const {
  ...
  fs::path base =
      repo_opened_ ? (repo_.root() / "services") : fs::path("services");
  std::error_code ec;
...
bool Supervisor::stop_unit(const Unit &u, ProcState *out_state) {
  ...
  auto s = ProcessRunner::status(u, run_dir_);
  if (out_state)
    *out_state = {s.pid, s.running, s.last_exit_code};
  return ok;
}

std::vector<Unit>
Supervisor::load_units_by_names(const std::vector<std::string> &names) const {
  std::vector<Unit> v;
...
DepGraph Supervisor::build_dep_graph(const std::vector<Unit> &units) const {
  ...
  for (const auto &u : units) {
    g.try_emplace(u.name(), std::vector<std::string>{});
  }
  for (const auto &u : units) {
    auto &edges = g[u.name()];
...
void Supervisor::monitor_exit_loop(Unit u) {
  ...
  while (!stop_flag_) {
    ...
    if (!st.running) {
      ...
      if (u.restart == RestartPolicy::Always)
        should_restart = true;
      ...
      if (should_restart) {
        ...
        while (!restarts.empty() &&
               (now - restarts.front()) >
                   std::chrono::seconds(u.restart_window_sec)) {
          restarts.pop_front();
        }
        restarts.push_back(now);
        if ((int)restarts.size() > u.max_restarts_in_window) {
          spdlog::warn("[unit={}] too many restarts; backing off {}s", u.name(),
                       u.restart_window_sec);
...
static bool http_expect_ok(int code, const std::string &expect) {
  ...
  if (expect.find(',') != std::string::npos) {
    ...
    while (std::getline(ss, tok, ',')) {
      ...
      if (tok.find('-') != std::string::npos) {
        int a = 0, b = 0;
        std::sscanf(tok.c_str(), "%d-%d", &a, &b);
        if (code >= a && code <= b)
          return true;
      } else {
        int v = 0;
        std::sscanf(tok.c_str(), "%d", &v);
        if (code == v)
          return true;
      }
    }
  ...
  if (expect.find('-') != std::string::npos) {
    int a = 0, b = 0;
    std::sscanf(expect.c_str(), "%d-%d", &a, &b);
    return code >= a && code <= b;
  }
  int v = 0;
  std::sscanf(expect.c_str(), "%d", &v);
  return code == v;
}

static std::optional<int> http_get_status(const std::string &url,
                                          int timeout_ms) {
...
void Supervisor::health_loop(Unit u) {
  while (!stop_flag_) {
    ...
    if (have_script) {
      pid_t pid = ::fork();
      if (pid == 0) {
        if (!u.working_dir.empty())
          ::chdir(u.working_dir.c_str());
        std::vector<char *> argv;
        argv.reserve(u.exec_health.size() + 1);
        for (auto &s : u.exec_health)
    ...
    if (have_http) {
      auto stcode =
          http_get_status(u.health_http_url, u.health_http_timeout_ms);
      ok_http =
          (stcode.has_value() && http_expect_ok(*stcode, u.health_http_expect));
    }

    bool have_any = have_script || have_http;
    bool healthy =
        (!have_any) || ((!have_script || ok_script) && (!have_http || ok_http));

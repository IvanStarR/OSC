...
static int
exec_shell_env(const std::string &cmd, const fs::path &cwd,
...
int ProcessRunner::start(const Unit &u, const fs::path &run_dir,
  ...
  for (const auto &cmd : u.exec_start_pre) {
    ...
    int rc = exec_shell_env(
        cmd, u.working_dir.empty() ? u.path.parent_path() : u.working_dir,
        u.env);
    if (rc != 0) {
  ...
  if (const char *env = ::getenv("GITPROC_LOG_MAX_MB")) {
    ...
    if (v >= 0)
      max_mb = static_cast<std::uintmax_t>(v);
  }
  ...
  if (n > 0) {
    ...
    spdlog::error("[unit={}] exec failed (errno={}): {}", name, child_errno,
  ...
  if (!u.pid_file.empty()) {
    ...
    auto deadline = std::chrono::steady_clock::now() +
                    std::chrono::seconds(u.pidfile_timeout_sec);
    ...
    if (final_pid != pid) {
      ...
      spdlog::warn("[unit={}] PIDFile not found/updated in {}s; continue with "
                   "fork pid={}",
...
bool ProcessRunner::stop(const Unit &u, const fs::path &run_dir) {
    ...
    if (rc == 0) {
      ...
      while (waited < u.timeout_stop_sec) {
        ...
        if (r == *pid) {
          ...
          StateStore::write_status_json(StateStore::status_file(name, run_dir),
                                        *pid, exit_code);
          spdlog::info("[unit={}] stopped via ExecStop exit={}", name,
                       exit_code);
          ::unlink(pidf.c_str());
          return true;
        }
  ...
  spdlog::info("[unit={}] stopping pid={} (SIGTERM, timeout={}s)", name, *pid,
               u.timeout_stop_sec);
  ::kill(*pid, SIGTERM);
  ...
  int status = 0;
  ::waitpid(*pid, &status, 0);
  StateStore::write_status_json(StateStore::status_file(name, run_dir), *pid,
                                137);
  ::unlink(pidf.c_str());
  return true;
}
...
ProcStatus ProcessRunner::status(const Unit &u, const fs::path &run_dir) {
  ...
  if (pid && pid_alive(*pid)) {
    ...
    s.running = false;
    s.pid = pid.value_or(-1);
    s.last_exit_code = 0;
  }
  return s;
}

bool ProcessRunner::reload(const Unit &u, const fs::path &run_dir,
                           const fs::path &logs_dir) {
  (void)logs_dir;
  const auto name = u.name();
  const auto pidf = StateStore::pid_file(name, run_dir);
  ...
  int rc = exec_shell_env(
      cmd, u.working_dir.empty() ? u.path.parent_path() : u.working_dir, u.env);
  if (rc == 0) {
    spdlog::info("[unit={}] reloaded via ExecReload", name);
    return true;
  } else {
    spdlog::warn("[unit={}] ExecReload failed (rc={}); fallback to restart",
                 name, rc);
    (void)stop(u, run_dir);
    return start(u, run_dir, logs_dir) == 0;
  }

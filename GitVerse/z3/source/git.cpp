...
static std::string join_shell(const std::vector<std::string>& args){
  ...
  for (size_t i=0;i<args.size();++i){
    if (i) oss << ' ';
    for(char c: args[i]){
      if (c==' '||c=='"'||c=='\'') oss<<'\\'<<c; else oss<<c;
    }
  }
  return oss.str();
}

int GitRepo::exec_cmd(const std::vector<std::string>& args,
                      const fs::path& cwd,
                      std::string* out){
...
bool GitRepo::looks_like_url(const std::string& s){
  return s.rfind("http://",0)==0 || s.rfind("https://",0)==0 || s.rfind("git@",0)==0 || s.rfind("ssh://",0)==0;
}
...
std::string GitRepo::file_url_to_path(const std::string& url){
  ...
  if (!looks_like_file_url(url)) return url;
...
GitRepo GitRepo::open(const std::string& path_or_url, const fs::path& work_base){
  ...
  if (!fs::exists(dest / ".git")) {
    ...
    std::string src = looks_like_file_url(path_or_url) ? file_url_to_path(path_or_url) : path_or_url;
    std::string out;
    int rc = exec_cmd({"git","clone","--depth","1",src,dest.string()}, fs::current_path(), &out);
    if (rc != 0) throw std::runtime_error("git clone failed: " + out);
  }
...
std::optional<std::string> GitRepo::current_commit() const {
  if (!fs::exists(root_ / ".git")) return std::nullopt;
  std::string out;
...
bool GitRepo::pull(const std::string& branch) {
  if (!fs::exists(root_ / ".git")) return false;
  ...
  int rc = exec_cmd({"git","fetch","--depth","1","origin",branch}, root_, &out);
  if (rc != 0) {
    pull_failures_ = std::min(pull_failures_ + 1, 10);
    int backoff = std::min(30, 1 << std::min(pull_failures_, 5)); // 1,2,4,8,16, then 30
  ...
  if (head_changed) last_head_ = after;
  return head_changed;
}
...
bool GitRepo::has_unit_changed(const fs::path& unit_path, bool* head_changed) {
  if (head_changed) *head_changed = false;

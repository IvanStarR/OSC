...
static void load_env_file(Unit &u, const fs::path &file) {
  ...
  if (!in) {
    spdlog::warn("[unit={}] EnvironmentFile not found: {}", u.name(),
                 file.string());
    return;
  }
  std::string line;
...
static void append_name_list(std::vector<std::string> &dst,
                             const std::string &val) {
  std::stringstream ss(val);
  std::string one;
  while (std::getline(ss, one, ';')) {
...
Unit Unit::Load(const fs::path &p) {
  Unit u;
  u.path = fs::absolute(p);
  std::ifstream in(u.path);
  if (!in)
    throw std::runtime_error("Unit file not found: " + p.string());
    ...
    if (key == "ExecStart") {
      ...
      if (!pf.is_absolute()) {
        if (!u.working_dir.empty())
          ...
          pf = u.path.parent_path() / pf;
      }
      u.pid_file = fs::absolute(pf);

    } else if (key == "PIDFileTimeoutSec") {
      try {
        u.pidfile_timeout_sec = std::stoi(val);
      } catch (...) {
      }

    } else if (key == "TimeoutStopSec") {
      try {
        u.timeout_stop_sec = std::stoi(val);
      } catch (...) {
      }
      ...
      try {
        u.health_http_timeout_ms = std::stoi(val);
      } catch (...) {
      }
      ...
      try {
        u.restart_sec = std::stoi(val);
      } catch (...) {
      }

    } else if (key == "RestartWindowSec") {
      try {
        u.restart_window_sec = std::stoi(val);
      } catch (...) {
      }

    } else if (key == "MaxRestartsInWindow") {
      try {
        u.max_restarts_in_window = std::stoi(val);
      } catch (...) {
      }
  ...
  if (u.exec.empty())
    throw std::runtime_error("ExecStart is required");
  if (!u.working_dir.empty() && !fs::exists(u.working_dir))
    throw std::runtime_error("WorkingDirectory not found: " +
                             u.working_dir.string());

  for (auto &ef : u.env_files)
    load_env_file(u, ef);

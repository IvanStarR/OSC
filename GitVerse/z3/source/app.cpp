...
static void tail_file(const fs::path& p, int lines){
  ...
  if (!in) { std::cout << "(no file: " << p.string() << ")\n"; return; }
  in.seekg(0, std::ios::end);
  auto pos = in.tellg();
  int count = 0; std::string buf;
  for (long long i = static_cast<long long>(pos)-1; i>=0; --i){
    in.seekg(i);
    char c; in.get(c);
    if (c=='\n') {
      if (++count > lines) { in.seekg(i+1); break; }
    }
  }
  while (std::getline(in, buf)) std::cout << buf << "\n";
}

static Unit load_unit_from(const std::optional<RepoUnit>& ru, const std::optional<Target>& tgt) {
  if (ru) {
...
int App::run(int argc, char** argv) {
  ...
  if (!pr.cmd) {
    if (!pr.error.empty()) spdlog::error("{}", pr.error);
    print_help();
  ...
  return std::visit([&](auto&& c)->int {
    ...
    if constexpr (std::is_same_v<T, CmdHelp>) {
      ...
      std::cout << fmt::format("gitproc {} ({}, built {})\n",
        GITPROC_COMMIT, GITPROC_BRANCH, GITPROC_BUILD_TIME);
      return 0;
      ...
      if (c.json.value_or(false)) {
        std::cout << fmt::format(R"({{"running":{},"pid":{},"last_exit_code":{}}})",
          st.running ? "true" : "false", st.pid, st.last_exit_code) << "\n";
      } else {
      ...
      if (!fs::exists(svc)) { std::cout << "(no services/)\n"; return 0; }
      ...
      auto print_tail = [&](const fs::path& p, int lines){
        std::cout << "=== " << p.string() << " (last " << lines << " lines)\n";
        tail_file(p, lines);
      };
      ...
      if (c.follow) {
        std::error_code ec;
        std::uintmax_t last_out = fs::exists(outp,ec)?fs::file_size(outp,ec):0;
        std::uintmax_t last_err = fs::exists(errp,ec)?fs::file_size(errp,ec):0;
        while (true) {
      ...
      auto start_or_log = [&](const fs::path& up){
        try {
          Unit u = Unit::Load(up);
          if (!sup.start(u)) { spdlog::error("[run] failed to start"); }
      ...
      spdlog::info("[run] repo={} unit={} branch={} period={}ms",
                   repo.root().string(), c.unit, c.branch, c.autosync_sec*1000);
